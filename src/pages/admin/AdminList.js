...
function AdminList() {
    ...
    const fetchData = async () => {
        const result = await apiFactory.adminApi.getList({ limit, page, search: search })
        setAdminList(result?.data?.items?.map((e, i) => (
            {
                id: e?.id,
                key: e?.id,
    ...
    const onDelete = async () => {
        setLoading(true)
        const result = await apiFactory.adminApi.delete(selectedRowKeys)
        setLoading(false)
