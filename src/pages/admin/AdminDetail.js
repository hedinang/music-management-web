...
function AdminDetail({ different }) {
    ...
    const onFinish = async (values) => {
        ...
        if (different.type === 'edit') {
            result = await apiFactory.adminApi.update({
                id: param?.id,
                name: values?.name,
        ...
        if (different.type === 'add') {
            result = await apiFactory.adminApi.create({
                name: values?.name,
                username: values?.username,
                password: values?.password,
                image: values?.img?.file,
    ...
    const onDelete = async () => {
        setLoading(true)
        const result = await apiFactory.adminApi.delete([param.id])
        setLoading(false)
        if (result.status === 200) {
    ...
    const fetchData = async () => {
        if (param.id) {
            const result = await apiFactory.adminApi.getById(param.id)
            if (result.data) {
                setInitialData({
        ...
        >
            ...
            <StickyFooter >
                <div className="flex justify-between gap-[5px]">
                    <Button className='bg-[#868e96] text-white ml-[230px]' onClick={() => navigate('/admin/list')}>Quay lại</Button>
                    <div className='flex gap-[5px]'>
                        {different.type !== 'view' && <Button className='ml-auto bg-[#007dce] text-white' htmlType="submit">Lưu</Button>}
                        {different.type === 'view' && <Button className='ml-auto bg-[#aec57d] text-white' onClick={() => navigate(`/admin/edit/${param.id}`)}>Sửa</Button>}
